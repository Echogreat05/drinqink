...
-- Adds support for: group orders, event planner, enhanced loyalty,
-- referrals, subscriptions, analytics, notifications, favorites,
-- corporate workflows, and customizations
-- ============================================================
...
-- ============================================================
-- 3. ENHANCED LOYALTY POINTS SYSTEM
-- ============================================================
...
-- Update loyalty_points table to include current balance
...
-- ============================================================
-- 7. ENHANCED DISPUTES - THREADED COMMENTS
-- ============================================================
...
-- ============================================================
-- 10. FAVORITES (SAVED VENDORS/PRODUCTS)
-- ============================================================
...
-- ============================================================
-- 11. ENHANCED CORPORATE ACCOUNTS
-- ============================================================
...
-- ============================================================
-- ROW LEVEL SECURITY POLICIES (NEW TABLES)
-- ============================================================

-- group_orders
CREATE POLICY "Group orders viewable by members" ON public.group_orders FOR SELECT USING (
  auth.uid() = organizer_id
  OR EXISTS (SELECT 1 FROM public.group_order_members WHERE group_order_id = id AND member_id = auth.uid())
...
CREATE POLICY "View group members via order" ON public.group_order_members FOR SELECT USING (
  EXISTS (
    ...
    WHERE id = group_order_id AND (organizer_id = auth.uid() OR id IN (
      SELECT group_order_id FROM public.group_order_members WHERE member_id = auth.uid()
    ))
  )
);
...
CREATE POLICY "View event plan items via plan" ON public.event_plan_items FOR SELECT USING (
  EXISTS (
    SELECT 1 FROM public.event_plans WHERE id = event_plan_id AND customer_id = auth.uid()
  )
);
...
CREATE POLICY "Admins manage loyalty transactions" ON public.loyalty_transactions FOR ALL USING (public.has_role(auth.uid(), 'admin'));
...
CREATE POLICY "Users manage own notification channels" ON public.notification_channels FOR ALL USING (auth.uid() = user_id);

-- notification_preferences
CREATE POLICY "Users manage own notification preferences" ON public.notification_preferences FOR ALL USING (auth.uid() = user_id);

-- notification_sent_log
CREATE POLICY "Users view own notification logs" ON public.notification_sent_log FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Admins view all notification logs" ON public.notification_sent_log FOR SELECT USING (public.has_role(auth.uid(), 'admin'));

-- favorites
CREATE POLICY "Users manage own favorites" ON public.favorites FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "View favorites public" ON public.favorites FOR SELECT USING (true);

-- corporate_team_members
CREATE POLICY "Corporate team view members" ON public.corporate_team_members FOR SELECT USING (
  EXISTS (SELECT 1 FROM public.corporate_accounts WHERE id = corporate_account_id AND owner_id = auth.uid())
  OR auth.uid() IN (SELECT user_id FROM public.corporate_team_members WHERE corporate_account_id = corporate_team_members.corporate_account_id)
);
...
CREATE POLICY "Approvers manage approvals" ON public.corporate_approvals FOR UPDATE USING (
  EXISTS (SELECT 1 FROM public.corporate_team_members WHERE corporate_account_id = corporate_account_id AND user_id = auth.uid() AND is_approver = true)
);

-- corporate_budgets
CREATE POLICY "Corporate team view budgets" ON public.corporate_budgets FOR SELECT USING (
  EXISTS (SELECT 1 FROM public.corporate_accounts WHERE id = corporate_account_id AND owner_id = auth.uid())
  OR auth.uid() IN (SELECT user_id FROM public.corporate_team_members WHERE corporate_account_id = corporate_budgets.corporate_account_id)
);

-- corporate_activity_log
CREATE POLICY "Corporate team view activity" ON public.corporate_activity_log FOR SELECT USING (
  EXISTS (SELECT 1 FROM public.corporate_accounts WHERE id = corporate_account_id AND owner_id = auth.uid())
);
